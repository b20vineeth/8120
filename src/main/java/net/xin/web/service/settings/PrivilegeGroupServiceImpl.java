...
import net.xin.web.application.settings.PrivilegeGroupApp;
...
@Component
public class PrivilegeGroupServiceImpl implements PrivlegeGroupService {

	@Autowired
	private PrivilegeGroupApp app;
	 
	 

	@Override
	public ValidationForm savePrivilegeGroup(PrivilegeTypeForm form) throws PrevilegeException,BussinessException, BusinessViolatonException {
		try
		{
			ValidationForm obj=new FormValidation().validate(form);
			if(obj.isResult())
			{
				return app.SavePrivilegeGroup(form);
			}
			else
			{
				return obj; 
